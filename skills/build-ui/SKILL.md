...
> Chrome CDP（ポート 9222）は Phase 0 で自動起動されるため、手動起動は不要。

## ワークフロー

### Phase 0: 初期化

1. **Chrome CDP プリフライトチェック**:
   a. `curl -s --max-time 2 http://localhost:9222/json/version` で確認
   b. 失敗 → `bash scripts/ensure-chrome-cdp.sh` を実行
   c. それでも失敗 → ユーザーに手動起動を案内して停止
2. `.build-ui/` ディレクトリと `screenshots/` サブディレクトリを作成
3. dev server が起動しているか確認。未起動なら起動する
4. Playwright MCP の `browser_navigate` で対象ページにアクセスし、MCP 接続が正常か確認（統合テスト）
5. ユーザーの要件を確認（何を作る/改善するか）
