...
`/build-ui` スキルを使う場合、Phase 0 で自動的に Chrome CDP が起動される。手動起動は不要。

また、以下のスクリプトを直接実行することもできる:

```bash
bash scripts/ensure-chrome-cdp.sh
```

このスクリプトはポート 9222 をチェックし、Chrome が未起動なら自動で起動してヘルスチェックまで行う。

**手動起動:**

自動起動を使わない場合、以下のコマンドで Chrome を起動する:
